...
DATA_DIR = os.path.join(HOME_DIR, ".config", "kimsystemai")
...
# Initialize KimSystem AI Client
...
def get_system_instruction(yolo=True):
    ...
    return f"""
You are KimSystem AI CLI, a specialized expert in Arch Linux and the Hyprland window manager.
...
def process_chat(session_id, user_text, yolo=True, screenshot=False, model="gemini-2.0-flash"):
    ...
    if screenshot:
        ...
        if img_bytes:
            ...
            print("[System] Screenshot attached.")
    ...
    try:
        ...
        if command_executed:
            ...
            print("\n[System] Summarizing results...")
            second_response = chat.send_message(f"Command execution results: {combined_command_output}. Summarize what you did in clean human language.")
            final_human_text = second_response.text
            print(f"\n---\n{final_human_text}")
            full_text += f"\n\n---\n{final_human_text}"
...
def main():
    ...
    if args.prompt:
        ...
        # Interactive mode
        print(f"KimSystem AI CLI - Model: {args.model} - YOLO: {args.yolo}")
